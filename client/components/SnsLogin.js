...
function SnsLogin({navigation, setNaverLoginLink, naverLoginLink, setIsSnsLogin}) {
  ...
  const naverLogin = async () => {
    const getNaverLoginLink = async () => {
      ...
      //   // gpt가 짜준 코드
      //   const userResponse = await fetch('http://192.168.200.17:5300/naverlogin/user', {
      //   method: 'GET',
      //   headers: {
      ...
      // setUserInfo(userData); // userInfo를 상위 스코프에서 접근 가능한 변수에 할당
 ...
 //  초기에 잘못짠 코드 : 비동기 처리 안되고 조잡함
      ...
      console.log('프론트엔드 테스트(프론트35줄):', userInfo);

      const listAllUsers = await fetch('http://192.168.200.17:5300/firebaseLogin')
      const userList = await listAllUsers.json()
      console.log('유저리스트(프39줄) :', userList)
